...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ roleRoute('leave-types.update', $leaveType->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name', $leaveType->name) }}" required>
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('code') is-invalid @enderror" id="code"
                                name="code" value="{{ old('code', $leaveType->code) }}" required>
                            @error('code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('annual_quota') is-invalid @enderror"
                                id="annual_quota" name="annual_quota"
                                value="{{ old('annual_quota', $leaveType->annual_quota) }}" min="0" required>
                            @error('annual_quota')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('accrual_type') is-invalid @enderror" id="accrual_type"
                                ...
                                <option value="yearly" {{ old('accrual_type', $leaveType->accrual_type) == 'yearly' ? 'selected' : '' }}>Yearly</option>
                                <option value="monthly" {{ old('accrual_type', $leaveType->accrual_type) == 'monthly' ? 'selected' : '' }}>Monthly
                                </option>
                                <option value="none" {{ old('accrual_type', $leaveType->accrual_type) == 'none' ? 'selected' : '' }}>None</option>
                            </select>
                            @error('accrual_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('max_carry_forward') is-invalid @enderror"
                                id="max_carry_forward" name="max_carry_forward"
                                value="{{ old('max_carry_forward', $leaveType->max_carry_forward) }}" min="0" required>
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_paid" name="is_paid" value="1" {{ old('is_paid', $leaveType->is_paid) ? 'checked' : '' }}>
