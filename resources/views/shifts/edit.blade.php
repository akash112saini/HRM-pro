            ...
            <div class="card">
                <div class="card-header bg-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Edit Work Shift</h5>
                ...
                <div class="card-body">
                    <form action="{{ roleRoute('shifts.update', $shift->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                @foreach(['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'] as $index => $day)
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="working_days[]" value="{{ $index }}" id="day_{{ $index }}" 
                                            {{ in_array($index, old('working_days', $shift->working_days ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('grace_period_minutes') is-invalid @enderror" id="grace_period_minutes" name="grace_period_minutes" value="{{ old('grace_period_minutes', $shift->grace_period_minutes ?? 15) }}" min="0">
